...
class FinDataLoader:
    ...
    def __call__(self,code,day_after):
        data = pd.read_csv(f"{self.path}/labeled/{day_after}/{code}.csv",index_col=[0])
    ...
    def labeling(self, day_after):
        ...
        if os.path.exists(f"{self.path}/labeled/{day_after}") is False:
            os.mkdir(f"{self.path}/labeled/{day_after}")
        ...
        for code,_ in tqdm(self.stock_list.items()):
            ...
            merged_data.dropna().to_csv(f"{self.path}/labeled/{day_after}/{code}.csv",encoding='utf-8-sig')
